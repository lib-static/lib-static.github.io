...
You can think of a [static website](https://en.wikipedia.org/wiki/Static_web_page) as a shared folder of readonly files exposed on the web.
The URL in your web browser is just a path to find a document in a folder, generally representing a request for an HTML file in a specific location on the server.
...
- faster performance (easy caching / CDN, low bandwidth, no processing time, ability to handle high traffic / scale).
